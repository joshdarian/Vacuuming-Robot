...
class RectangularRoom(object):
    ...
    def isTileCleaned(self, m, n):
        ...
        self.m=m
        self.n=n
        if (self.m,self.n) in self.clean:
            return True
        else:
            return False
...
class StandardRobot(Robot):
    ...
    def updatePositionAndClean(self):
        ...
        newPos=curPos.getNewPosition(self.getRobotDirection(), self.speed)
        if self.room.isPositionInRoom(newPos)==True:
            self.setRobotPosition(newPos)
            if self.room.isTileCleaned(int(newPos.getX()), int(newPos.getY()))==False:
...
def runSimulation(num_robots, speed, width, height, min_coverage, num_trials,
    ...
    for trial in range(num_trials):
        room=RectangularRoom(width, height) #can't be instantiated outside
        for index in range(num_robots): #because every trial requires a new room,
            robots.append(robot_type(room, speed)) #robots follow as require room
        steps=0
